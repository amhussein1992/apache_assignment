...
1. **Start MySQL using Docker:**
   ```sh
   docker run --name devops-mysql -e MYSQL_ROOT_PASSWORD=admin -e MYSQL_DATABASE=devopsdb -p 3306:3306 -d mysql:8.0
   ```
...
2. **Update your `config.py`:**
   ...
   ```python
   SQLALCHEMY_DATABASE_URI = "mysql+pymysql://root:admin@localhost:3306/devopsdb"
   ```
